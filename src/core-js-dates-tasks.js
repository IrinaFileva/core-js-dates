...
function dateToTimestamp(date) {
  const startData = new Date('01 Jan 1970 00:00:00 UTC');
  return new Date(date) - startData;
}
...
function getNextFriday(date) {
  ...
  const dayDifference = dayNumber < 5 ? 5 - dayNumber : 7 - dayNumber + 5;
  const friday = new Date(date.setDate(date.getDate() + dayDifference));
  return friday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.ceil(varianceMs / msInDay) + 1;
}
...
function getNextFridayThe13th(date) {
  const msInDay = 24 * 60 * 60 * 1000;
  let newDate = date;
  let msInDate = date.getTime();

  while (!(newDate.getDay() === 5 && newDate.getDate() === 13)) {
    msInDate += msInDay;
    newDate = new Date(msInDate);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const msInEnd = new Date(end).getTime();
  let msInStart = new Date(start).getTime();
  const daysInPeriod = (msInEnd - msInStart) / msInDay;
  const allDays = [];

  for (let i = 1; i <= daysInPeriod + 1; i += 1) {
    allDays.push(new Date(msInStart).toISOString().split('T')[0]);
    msInStart += msInDay;
  }
  let count = 0;
  const result = [];
  for (let i = 0; i < allDays.length; i += 1) {
    ...
    if (count <= countWorkDays) {
      const day = allDays[i].split('-').reverse().join('-');
      result.push(day);
    } else {
      i += countOffDays - 1;
      count = 0;
